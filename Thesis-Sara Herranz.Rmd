...
avg_pred_women
quantile75
bot <- bot |> rename(quantile.resident.collect.each.prediction = quantile25,
              X0.25. = quantile50,
              quantile.resident.collect.each.prediction.1 = quantile75,
              X0.5. = avg_pred_women) |> 
  dplyr::select(-quantile.resident.collect.each.prediction.2, -X0.75., -mean..resident.where..each.gender...0...collect.each.prediction.)
...
tengo que descubrir bien como interpretar gen_ins, qué significa valor positivo y qué significa valor negativo. Hecho eso, podré identificar los coeficientes de la regresión, porque no tiene sentido que género, ser mujer reduzca -0.33 gen_ins:

**Component 1 (general urban insecurity)** is anchored by both **specific spatial perceptions** (isolated streets and parks, commercial areas, and nightlife zones) and **generalized urban insecurity**. The inverse loadings from p10_1, p10_2, p11_1r, and p11_2r (which are inverse because the item is framed in terms of **feeling safe**) indicate that **higher scores on this component correspond to stronger feelings of insecurity** in both local and city-level settings.

```         
gender       -0.4866663  0.0703552  -6.917 9.41e-12 ***
```

RESUELTO: las variables binarias se han codificado al revés: el 1 sigue siendo 1 y el 2 se convierte en 0, cuando debería haber sido 1 a 0 y 2 a 1.

"La variable `gender` muestra un efecto significativo sobre la inseguridad percibida (`gen_ins`). Con el grupo de referencia siendo las mujeres, los hombres presentan una percepción significativamente menor de inseguridad (β = −0.374, p \< 0.001), controlando por el resto de factores incluidos en el modelo."
