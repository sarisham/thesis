---
title: "Thesis: Safety perception in urban spaces, an ABM approach"
...
The Pamplona dataset, originally imported from SPSS, contains variables with labelled values. Labelled variables hold both numeric codes and descriptive labels, with codes representing categories (e.g., “1 = Male”, “2 = Female”). Using `mutate` with `across(where(is.labelled), ~ as_factor(.))`, these labelled variables are converted to R factors, allowing us to retain their descriptive labels. Converting labelled variables in this way ensures that categorical data is treated appropriately in analysis and allows for descriptive interpretation.
...
Attending the plot results, the CART imputation performs well across both categorical and continuous variables, preserving original distributions with minimal distortion, thus being a robust method for handling our missing data.
...
Then, we compare the mean and standard deviation of `age` in the original and imputed datasets. The imputed values are close to the original statistics, suggesting that the imputation method has maintained the underlying distribution characteristics effectively.
...
I use `compare_original_imputed_normalized` to plot overlapping histograms for the original and imputed datasets, so I can evaluate whether the imputed values accurately reflect the original data. Normalizing both distributions ensures that differences in sample sizes do not skew the comparison, providing a visual representation of how closely the imputed values replicate the original data distribution.

Attending to the results of the plot, the distributions of both the original data and the imputed one is almost the same, with slight differences, indicating that CART effectively preserves statistical characteristics of the data.
...
Instead of applying the function to the whole dataset at the same time, specific subsets of variables are analyzed to focus on particular themes within the dataset. This allows for an organized examination of conceptually related variables.
...
```{r}
# Apply the recoding: change 1 (Yes) to 1 and 2 (No) to 0 in pvictimisation_vars
pvictimisation_vars[] <- lapply(pvictimisation_vars, 
                                function(x) ifelse(x == 1, 1, 0))

# Synthetic index (unweighted): 1 if at least one victimisation reported, 0 if none
...
+============+==================================+===================================+===========================+===================+
| p5_1       | Theft w/o violence (bag, etc.)   | Hurto \>400€                      | 1                         | 4                 |
+------------+----------------------------------+-----------------------------------+---------------------------+-------------------+
...
+------------+----------------------------------+-----------------------------------+---------------------------+-------------------+
| p5_8       | Fraud / scam                     | Estafa                            | 1                         | 4                 |
+------------+----------------------------------+-----------------------------------+---------------------------+-------------------+
| p6_1       | Physical aggression              | Lesiones                          | ≈1.1                      | ≈4.4              |
+------------+----------------------------------+-----------------------------------+---------------------------+-------------------+
| p6_2       | Threats or coercion              | Coacción / amenazas               | \~1.0 (if available)      | 4                 |
+------------+----------------------------------+-----------------------------------+---------------------------+-------------------+
...
While both the binary and the weighted approaches are methodologically valid, the latter provides a more detailed representation of victimization experiences. It also allows us to explore whether more severe or repeated victimization exerts a stronger influence on perceived insecurity. This distinction is not merely conceptual, as it may affect model performance, explanatory power, and ultimately, the conclusions.
...
# Visually checking distribution
...
After carrying out the different analysis, running the CATPCA models and observed the results, it has been proven that the **quartile based encoding was the best performing recode method**:
...
One of the key methodological decisions in principal component analysis is determining how many components to retain. In this study, the first two components account for approximately **63.4% of the total variance**, while the inclusion of a third component raises that figure to **72.9%**.

**Retaining only two components** provides a more parsimonious model and facilitates interpretation and integration into subsequent models, as we plan on doing with the regressions and the ABM. However, it entails discarding potentially meaningful variation. On the other hand, **retaining three components**, allows for a more nuanced understanding of the underlying structure of perceived insecurity. It distinguishes between different domains of perception (personal, institutional, and environmental) and better reflects the complexity of the phenomenon, though it introduces additional interpretive and computational layers. I have opted to retain only two components, since the total variance they account for is statistically enough and, for the present research, I prioritize a parsimonious model rather than a complex one.
...
As stated earlier, I chose the first two components.
...
Given that these components represent distinct yet potentially related aspects of the underlying constructs, it is appropriate to use them simultaneously as dependent variables in a **multivariate multiple regression framework**. This approach allows me to assess how a set of predictors influences each component, while also testing for overall effects across the multivariate outcome space.

## Processing predictors before the regressions

Before running the multiple regression analysis, I recoded the sociodemographic variables (income, education, and employment) to reduce the number of categories. This ensures model stability and improve interpretability, as categorical predictors with many levels can lead to sparse data in some categories, inflating standard errors and reducing statistical power.
...
While developing the model in GAMA, I noticed that the shapefile used to represent Pamplona (downloaded directly from the official website of the Pamplona city center) included only 13 neighborhoods, whereas the survey data referenced 14. The neighborhood Azpilagaña was missing from the shapefile so, due to geographical proximity, it was merged with Milagrosa-Arrosadia in all the variables related to neighborhood identification (`barrio`, `p13a`, and `p13b`).
...
Now, I ensure that each variable is appropriately encoded according to its measurement level. Ordinal variables, such as attitudes and sociodemographic indicators like education and income, are converted into ordered factors. Binary variables are recoded into a 0/1 format. Continuous variables are standardized using Z-scores, allowing for comparability across predictors with different scales and improving model convergence.
...
Such as trust in institutions or perceived environmental degradation, which represent broader cognitive evaluations or interpretations of one’s environment. **While they may not be directly simulated in the ABM, they are essential for capturing higher-order cognitive processing and may inform the probabilistic decision rules governing agent behavior.**
...
From the results obtained, the forward-selected models showed a better performance for both the general insecurity (`gen_ins`, R-squared of 0.42, RMSE 0.76, MAE 0.62) and the institutional security (`inst_ins`, R-squared 0.07, RMSE 0.98, MAE 0.77). Therefore, this set of predictors was the one chosen for our final model.
...
```{r}
modelo_mv <- lm(cbind(gen_ins, inst_ins) ~ agez + gender  + income + nationality + employment + victimized + p7_1 + p7_8 + p10_3 + p10_4 + p10_6 + p12 + p15 + p16 + p13a + p13b + p14z + p7_7 + p8 + p9, data = pampre) # este modelo es incluso mejor que el de antes, pero si lo paso por CV es peor... asi q mejor me quedo como estoy + p13a*gender + p10_3*barrio + p7_7*p14z

summary(modelo_mv)
```

The multivariate tests (Wilks’ Lambda and Pillai’s Trace) assess whether each independent variable has a **statistically significant effect on the system of dependent variables** (`gen_ins` (general insecurity) and `inst_ins` (institutional insecurity)). Unlike univariate regressions, these tests consider the **correlation structure between the dependent variables** and provide a more global assessment. In both tests, almost all variables exhibit statistically significant multivariate effects, except for `employment`, `p10_4` (opinion on security cameras), `p16` (past experience of neighborhood conflicts) and `p9` (changing neighborhoods for safety). On variable, `p13b` (rating of the most insecure neighborhoods) was significant only at p \< 0.1.
...
The first response variable, `gen_ins`, captures the emotional and spatial dimension of perceived insecurity. **Adjusted R² = 0.5069** indicates a strong explanatory power, and the model is statistically significant (**F(63, 764) = 14.49, p \< 2.2e-16**).

The second response variable, `inst_ins`, reflects insecurity in spaces that should be institutionally safeguarded (public services, transport, etc.). The model has lower explanatory power: although **adjusted R² = 0.174**, the model is still statistically significant (**F(63, 764) = 3.765, p = 2.2e-16**).
...
boot.ci(results, type = "perc", index = which(names(results$t0) == "p7_8.Q"))

```

# GAMA MODELING

The following elements are specific to the development of the agent-based model (ABM) in GAMA. They include key preparatory steps such as creating a stratified sample to reflect the demographic composition of the agents, fixing and cleaning the Pamplona shapefile (.shp) to ensure accurate spatial representation of neighbourhood boundaries, and computing a composite safety score for each neighbourhood based on survey data.

## Creating a stratified sample

Here, I prepare a synthetic sample of agents from survey data to be used in the ABM. It begins by selecting key sociodemographic and attitudinal variables, creating age categories, and drawing a stratified random sample of 20% from each age group within each neighborhood. Neighborhood identifiers are then replaced with meaningful names, including the fusion of two neighborhoods (8 and 9) into a single category, "Ermitagaña-Mendebaldea," since they are also grouped together in the shapefile that I will be using in GAMA. The resulting dataset is cleaned and exported as a CSV file.
...
table(agents$barrio_res)

```

### Extracting the parameters

In agent-based modeling, initializing agents with realistic and heterogeneous behavioral parameters is essential for capturing individual-level variability. Initially, I considered assigning each agent a predicted value of the `gen_ins` index based on a linear regression model, where each agent's perception of unsafety would be computed from the fixed coefficients of sociodemographic predictors (e.g., gender, age).

However, this approach only provides the population mean prediction, neglecting the uncertainty and heterogeneity present in the real data. Given the presence of heteroscedasticity in the residuals (as indicated by diagnostic tests), I opted for a more statistically sound method by leveraging bootstrapped coefficient distributions. This approach preserves the empirical variation of each coefficient and allows me to sample individualized parameters for agents from these distributions. It avoids the unrealistic assumption of homoscedastic and normally distributed errors, thereby providing a richer and more robust initialization of agent-specific values in the simulation.
...
Now, I construct an individual-level sensitivity index related to perceived insecurity when walking alone at night in dark areas. This variable is derived from the first component used in the CATPCA, where negative loadings indicate increased perceived insecurity. For interpretative consistency within the ABM, I invert the original signs, transforming them into positive weights so that higher values reflect greater sensitivity to darkness.

The responses to items p10_1 and p10_2—both ordinal variables measured on a four-point Likert scale—are rescaled onto a [0,1] interval, assuming a roughly **interval-like interpretation** of ordinal distances. Each item is then **weighted** according to its loading from the CATPCA model, and a **small individual error term** is incorporated (normally distributed with mean 0 and SD 0.025) to represent **unobserved heterogeneity** in agent-level sensitivity. The final index is a **linear combination** of the two weighted and perturbed components, yielding a continuous measure of perceived vulnerability to darkness.
...
#### Number of people that is "botellonero"

Now, I create `is_botellonero`, a variable indicating whether each individual is considered a "botellonero": a person who engages in group drinking in public spaces, commonly associated with youth social behavior in Spain.

To define the age range of potential *botelloneros* in the simulation model, I rely on the latest findings from the 2024 edition of the *Encuesta sobre Alcohol y Drogas en España (EDADES)*, published by the **Observatorio Español de las Drogas y las Adicciones (OEDA)**. According to this report, the practice of *botellón* is clearly influenced by age, with **30.8% of individuals aged 15 to 24 years** reporting participation in the past 12 months, it is a phenomenom becoming almost residual among older age groups, such as those between 55 and 64 years (0.6%). Based on this evidence, the model includes as potential *botelloneros* individuals aged **between 16 and 25 years**, allowing the simulation to realistically reflect the sociodemographic concentration of *botellón* participation in Spain.

The condition for being classified as a "botellonero" is twofold: the person must be between 14 and 24 years old, and must also fall within a 20% probability threshold.

> ```         
> Observatorio Español de las Drogas y las Adicciones (OEDA). (2024). Encuesta sobre Alcohol y Drogas en España (EDADES), 2023–2024: Informe. Ministerio de Sanidad. https://pnsd.sanidad.gob.es/profesionales/sistemasInformacion/sistemaInformacion/pdf/2024_Informe_EDADES.pdf
> ```

```{r}
#  ESTO HAY QUE QUITARLO PORQUE FINALMENTE NO SE HACE
# agents$is_botellonero <- ifelse(agents$age >= 15 & agents$age <= 24 & runif(nrow(agents)) < 0.2, TRUE, FALSE)
...
### Creating the crime index proxy

#### Composite safety score for each neighborhood

Now, I calculate a **composite safety score** for each neighborhood based on survey responses to two questions: which neighborhood is perceived as the safest (`p13a`) and which as the most unsafe (`p13b`). Since each respondent picked **only one barrio**, I treat the **number of times each barrio was chosen** as a proxy for positive perception (p13a) or a proxy for negative perception (`p13b`). Then, these counts can be normalized into proportions or scores, creating the composite score:​

$Safety Score= \frac{\text{Votes}_\text{safe}}{\text{Total Votes}} - \frac{\text{Votes}_\text{unsafe}}{\text{Total Votes}}$

The resulting score reflects overall public perception, with higher values indicating barrios perceived as safer.
...
barrio_scores <- barrio_scores |>
  mutate(
    real_crime_proxy_scaled = (real_crime_proxy - min(real_crime_proxy)) / 
                              (max(real_crime_proxy) - min(real_crime_proxy))
  )

barrio_clean <- barrio_scores |> 
  dplyr::select(barrio_id, safety_score, real_crime_proxy, real_crime_proxy_scaled)
...
The spatial data used in this simulation model comes from an `.osm` (OpenStreetMap) file obtained via the Google Street Maps API. This `.osm` file was first processed using **QGIS**, where the relevant **layers were extracted and exported as separate shapefiles**, specifically, buildings and roads. Once this was accomplished, I used R to clean and classify building data for better integration with the simulation. The aim is to **improve realism** in the model (e.g., residents live in actual residences, work in real workplaces) and enable functionalities like placing “botelloneros” in parks.

In addition, the neighborhood boundaries shapefile was obtained from the official website of the Pamplona City Council.

### Neighborhoods' limits

In this case, I opted to merge the neighborhoods of Azpilagaña and Milagrosa-Arrosadia from the shapefile used in the GAMA simulation model, as only one of them was included in the survey dataset. To ensure alignment between the spatial and survey data, I combined the missing neighborhood with an adjacent one that was present in the database. The decision was primarily based on **geographical proximity**, which ensures spatial continuity in the model. Additionally, I conducted a check of basic **sociodemographic indicators available at the official Pamplona's website** (such as age structure, educational attainment, migrant presence, and gender distribution). While indicators like youth population and the proportion of residents without formal education might suggest closer alignment with a different neighboring area, the overall profile across variables confirmed that Azpilagaña and Milagrosa-Arrosadia are more similar to each other than to the next alternative.
...
This chunk imports the building shapefile and classifies each polygon into one of three functional categories: "residence", "leisure", or "work_place".

```{r}
# Reading the shapefile containing buildings
edificios <- st_read("edif2.shp")

# Creating a new grouping variable to classify buildings by type
edificios <- edificios |> 
  mutate(bldng_t = case_when(buildng %in% c("apartments", "residential", "house",
                                            "semidetached_house", "detached") ~ "residence",
                             buildng %in% c("terrace", "greenhouse", "commercial",
                                            "religious", "church", "public", 
                                            "civic", "chapel", "cathedral", 
                                            "stadium") ~ "leisure",
                             buildng %in% c("industrial", "hotel", "hospital", "office",
                                            "college", "warehouse", "government",
                                            "kindergarten", "university",
                                            "fire_station", "townhall") ~ "work_place",
                             TRUE ~ buildng))

# Saving the updated shapefile with the buildings
# st_write(edificios, "edificios.shp", append = FALSE)

```

### Parks and squares: "parques"

I filter out buildings whose names indicate that they are parks or public squares. These areas are essential in the simulation as they are used to place nightlife groups (botelloneros).

```{r}
# Reading the shapefile containing parks
parques <- st_read("edif2.shp")
...
### Lightning scores per neighborhood

Here I construct a neighborhood-level lighting intensity index for Pamplona, in order to quantify how much lighting is present in each neighborhood, not just by counting the number of streetlights but by taking into account their electric power.

After arranging each streetlight to its corresponding neighborhood, I sum the power (`POTENCIA`) of all lamps within each neighborhood, giving a total light output per neighborhood (`pot_total`). Then, I normalize by area (in square kilometers) to get a lighting density per km² (`lum_por_km2`), which allows for comparability across neighborhoods of different sizes. Finally, a min-max normalization is applied to obtain an index (`lum_index_norm`) ranging from 0 to 1. The normalized lighting index is particularly appropriate in modeling frameworks where the relative spatial distribution of illumination is more meaningful than absolute power values. Moreover, its use in an ABM as mine, where I study perceived insecurity in public spaces, this standardized index facilitates comparative analysis across neighborhoods.

```{r}
# Load streetlight and neighborhood shapefiles
...
# Transform the streetlights layer to match the CRS of neighborhoods
farolas <- st_transform(farolas, st_crs(barrios))

# Perform spatial join to assign each lamp to its corresponding neighborhood polygon
farolas_por_barrio <- st_join(farolas, barrios, left = FALSE)

# Count the number of lamps per neighborhood
conteo_farolas <- farolas_por_barrio |> 
  count(BARRIO) 

# Calculate area (in km2) for each neighborhood
...
lum_area <- farolas_por_barrio |> 
  group_by(BARRIO) |> 
  # sum the power of all lamps (`pot_total`)
  ...
  # compute lighting per km2
  mutate(lum_por_km2 = pot_total / area_km2,
         # normalize it between 0 and 1
         lum_index_norm = (lum_por_km2 - min(lum_por_km2)) / (max(lum_por_km2) - min(lum_por_km2)))

```

# STOCHASTIC ANALYSIS

```{r convergence-plot, fig.width=8, fig.height=10, dpi=300}
...
par(mfrow = c(2, 3))  # diseño de 2 filas y 3 columnas

hist(stoch$insecure_rate, main = "Insecure Rate", xlab = "", col = "skyblue")
hist(stoch$avg_prediction, main = "Avg Prediction", xlab = "", col = "orange")
hist(stoch$stddev_prediction, main = "SD Prediction", xlab = "", col = "lightgreen")
hist(stoch$max_prediction, main = "Max Prediction", xlab = "", col = "tomato")
hist(stoch$avg_prediction_women, main = "Avg Prediction (Women)", xlab = "", col = "orchid")
...
```{r convergence-plot, fig.width=8, fig.height=10, dpi=300}
# Suponiendo que tu dataframe se llama `stoch` y tiene una fila por repetición
df_long_hist <- stoch %>%
    dplyr::select(-X) |> 
  pivot_longer(
    cols = everything(),
...
ggplot(df_long_hist, aes(x = value)) +
  ...
  facet_wrap(~ metric, scales = "free", nrow = 3, ncol = 2) +
  labs(x = NULL, y = "Count") +
...
Each panel shows the evolution of the cumulative mean for a specific output. The vertical red line indicates the 80th replication, after which all variables show minimal variation, suggesting convergence.

```{r convergence-plot, fig.width=9, fig.height=10, dpi=300}

# Suponiendo que tu data frame se llama `df` y no incluye la columna 'X'
df_long <- stoch %>%
  dplyr::select(-X) |> 
  mutate(replication = row_number()) %>%  # crea columna con el número de réplica
  pivot_longer(cols = -replication, names_to = "metric", values_to = "value") %>%
  group_by(metric) %>%
  mutate(cum_mean = cummean(value))  # calcula la media acumulada por métrica

ggplot(df_long, aes(x = replication, y = cum_mean)) +
  geom_line(color = "darkblue") +
    geom_vline(xintercept = 80, color = "red", linetype = "dashed", linewidth = 0.4) +  # línea roja vertical

  facet_wrap(~ metric, nrow = 3, ncol = 2, scales = "free_y") +
  ...
  theme(
    plot.margin = margin(t = 20, r = 20, b = 20, l = 20),
    strip.text = element_text(margin = margin(b = 20)),
    panel.spacing = unit(1.5, "cm"),  # aquí sí verás el cambio
    axis.title.y = element_text(size = 12, margin = margin(r =20)),
    axis.title.x = element_text(size = 12, margin = margin(t = 20)),
    axis.text.x = element_text(size = 10),
    axis.text.y = element_text(size = 10))
...
```{r}
bot0 <- read.csv("Thesis GAMA/models/sens_botellon_summary0.csv")
bot1 <- read.csv("Thesis GAMA/models/sens_botellon_summary1.csv")
bot2 <- read.csv("Thesis GAMA/models/sens_botellon_summary2.csv")
bot3 <- read.csv("Thesis GAMA/models/sens_botellon_summary3.csv")
bot4 <- read.csv("Thesis GAMA/models/sens_botellon_summary4.csv")

bot <- bind_rows(bot0, bot1, bot2, bot3, bot4)

# Renombrar columnas si es necesario
colnames(bot) <- c("nb_grupos_botellon", "insecure_rate", "avg_prediction", "stddev_prediction", "max_prediction", "avg_prediction_women")

# Transformar a formato largo
bot <- pivot_longer(
  bot,
  cols = -nb_grupos_botellon,
  names_to = "variable",
  values_to = "value"
)

# Gráfico combinado con todas las métricas
ggplot(bot, aes(x = nb_grupos_botellon, y = value, color = variable)) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, alpha = 0.4) +
  scale_y_continuous(breaks = seq(0, 3.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(0, 15, by = 3)) +
  labs(
    x = "Number of botellonero groups",
    y = "Mean value",
    color = NULL) +
...
ocio0 <- read.csv("Thesis GAMA/models/sens_ocio_summary0.csv")
ocio1 <- read.csv("Thesis GAMA/models/sens_ocio_summary1.csv")
ocio2 <- read.csv("Thesis GAMA/models/sens_ocio_summary2.csv")
ocio3 <- read.csv("Thesis GAMA/models/sens_ocio_summary3.csv")
ocio4 <- read.csv("Thesis GAMA/models/sens_ocio_summary4.csv")

ocio <- bind_rows(ocio0, ocio1, ocio2, ocio3, ocio4)

# Renombrar columnas si es necesario
colnames(ocio) <- c("probabilidad_ocio_fuera", "insecure_rate", "avg_prediction", "stddev_prediction", "max_prediction", "avg_prediction_women")

# Transformar a formato largo
ocio <- pivot_longer(
  ocio,
  cols = -probabilidad_ocio_fuera,
  names_to = "variable",
  values_to = "value"
)

# Gráfico combinado con todas las métricas
ggplot(ocio, aes(x = probabilidad_ocio_fuera, y = value, color = variable)) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, alpha = 0.4) +
  scale_y_continuous(breaks = seq(0, 3.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(0.1, 1.0, by = 2)) +
  labs(
    x = "probability of going out after work",
    y = "Mean value",
    color = NULL) +
  theme_minimal(base_size = 12) +
  theme(
...
threshold0 <- read.csv("Thesis GAMA/models/sens_threshold_summary0.csv")
threshold1 <- read.csv("Thesis GAMA/models/sens_threshold_summary1.csv")
threshold2 <- read.csv("Thesis GAMA/models/sens_threshold_summary2.csv")
threshold3 <- read.csv("Thesis GAMA/models/sens_threshold_summary3.csv")
threshold4 <- read.csv("Thesis GAMA/models/sens_threshold_summary4.csv")
threshold5 <- read.csv("Thesis GAMA/models/sens_threshold_summary5.csv")

threshold <- bind_rows(threshold0, threshold1, threshold2, threshold3, threshold4, threshold5)

# Renombrar columnas si es necesario
colnames(threshold) <- c("threshold_pred", "insecure_rate", "avg_prediction", "stddev_prediction", "max_prediction", "avg_prediction_women")

# Transformar a formato largo
threshold <- pivot_longer(
  threshold,
  cols = -threshold_pred,
  names_to = "variable",
  values_to = "value"
...
# Gráfico combinado con todas las métricas
ggplot(threshold, aes(x = threshold_pred, y = value, color = variable)) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, alpha = 0.4) +
  scale_y_continuous(breaks = seq(0, 3.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(0, 15, by = 3)) +
  labs(
    x = "threshold for classifying someone as insecure",
    y = "Mean value",
    color = NULL) +
  theme_minimal(base_size = 12) +
  theme(
...
```{r}
cw0 <- read.csv("Thesis GAMA/models/sens_crime_weight_summary0.csv")
cw1 <- read.csv("Thesis GAMA/models/sens_crime_weight_summary1.csv")
cw2 <- read.csv("Thesis GAMA/models/sens_crime_weight_summary2.csv")
cw3 <- read.csv("Thesis GAMA/models/sens_crime_weight_summary3.csv")
cw4 <- read.csv("Thesis GAMA/models/sens_crime_weight_summary4.csv")
cw5 <- read.csv("Thesis GAMA/models/sens_crime_weight_summary5.csv")

cw <- bind_rows(cw0, cw1, cw2, cw3, cw4, cw5)

# Renombrar columnas si es necesario
colnames(cw) <- c("crime_weight", "insecure_rate", "avg_prediction", "stddev_prediction", "max_prediction", "avg_prediction_women")
...
# Gráfico combinado con todas las métricas
ggplot(cw, aes(x = crime_weight, y = value, color = variable)) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, alpha = 0.4) +
  scale_y_continuous(breaks = seq(0, 3.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(0, 15, by = 3)) +
  labs(
    x = "crime weight parameter",
    y = "Mean value",
    color = NULL) +
  theme_minimal(base_size = 12) +
  theme(
...
cw0 <- read.csv("Thesis GAMA/models/sens_victimization_summary0.csv")
cw1 <- read.csv("Thesis GAMA/models/sens_victimization_summary1.csv")
cw2 <- read.csv("Thesis GAMA/models/sens_victimization_summary2.csv")
cw3 <- read.csv("Thesis GAMA/models/sens_victimization_summary3.csv")

cw <- bind_rows(cw0, cw1, cw2, cw3)

# Renombrar columnas si es necesario
colnames(cw) <- c("sens_victimization_summary", "insecure_rate", "avg_prediction", "stddev_prediction", "max_prediction", "avg_prediction_women")

# Transformar a formato largo
cw <- pivot_longer(
  cw,
  cols = -sens_victimization_summary,
  names_to = "variable",
  values_to = "value"
)

# Gráfico combinado con todas las métricas
ggplot(cw, aes(x = sens_victimization_summary, y = value, color = variable)) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, alpha = 0.4) +
  scale_y_continuous(breaks = seq(0, 3.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(0, 15, by = 3)) +
  labs(
    x = "sens_victimization_summary",
    y = "Mean value",
    color = NULL) +
  theme_minimal(base_size = 12) +
  theme(
