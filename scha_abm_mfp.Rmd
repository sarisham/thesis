---
title: "Thesis. Modeling perceived urban insecurity: an empirically grounded agent-based simulation"
...
```{r}
delitos <- pdf_text("PDF/BALANCE-CRIMINALIDAD-CUARTO-TRIMESTRE-2024.pdf")[468]
...
-   **P10 (statements of security and avoidance behaviors):**
    ...
    -   **P10_3 and P10_7**, which involve behavioral self-restraint (e.g., avoiding going out or visiting areas), are interpreted as **self-defense strategies** (behaviors likely resulting from prior insecurity). These were excluded to  avoid endogeneity and maintain conceptual clarity, since they are also behaviors that agents in the ABM may enact based on model parameters. However, these variables are still included in regression models as **predictors**, in order to assess their statistical association with perceived insecurity and to inform rule development in the ABM.

    -   **P10_4 to P10_6**, which capture attitudes towards surveillance technologies, were excluded, as they reflect political or normative beliefs rather than actual insecurity perceptions.

-   **P11 (global evaluations of safety):** both P11_1 and P11_2 provide scalar ratings of perceived safety in the respondent’s **neighborhood** and **city**, respectively. These are highly relevant and were included in the composite index.

-   **P12 (perceived neighborhood degradation and crime)** reflects the respondent’s perception of environmental disorder and local criminality. While not a direct emotional perception, it represents contextual perception of insecurity and is excluded.

-   **P8** (installing a security system) and **P9** (considering relocation) represent self-protective responses to perceived risk. As behavioral outcomes, they were excluded from the CATPCA to avoid conceptual circularity, but are included in regression models to assess their relationship with perceived and contextual insecurity.
...
agents <- pampre %>%
  ...
  mutate(gen_ins = (gen_ins - min(gen_ins, na.rm = TRUE)) / 
                   (max(gen_ins, na.rm = TRUE) - min(gen_ins, na.rm = TRUE))) |>  # Normalización 0–1
...
edificios <- st_read("NO USO/edif2.shp")
...
ggplot(ocio, aes(x = probabilidad_ocio_fuera, y = value, color = variable)) +
  ...
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, alpha = 0.4) +
  scale_y_continuous(breaks = seq(0, 3.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(0, 0.9, by = 0.3),  limits = c(0, 1)) +
  labs(
...
ggplot(threshold, aes(x = threshold_pred, y = value, color = variable)) +
  ...
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.4, alpha = 0.4) +
  scale_y_continuous(breaks = seq(0, 3.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(0, 1.0, by = 0.2), limits = c(0, 1)) +
  labs(
...
ggplot(cw, aes(x = sens_victimization_summary, y = value, color = variable)) +
  ...
  labs(
    x = "victimization weight parameter",
    y = "Mean value",
    color = NULL) +
  theme_minimal(base_size = 12) +
  theme(
