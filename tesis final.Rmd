...
> dplyr::select(gender, age, income, employment, nationality, victimized, barrio, gen_ins, p7_1, p7_8, p10_3, p10_4, p10_5, p10_6, p12, p15, p16, p13a, p13b, p18, p19, p7_7, p8, p9)
>
> mutate(age_group = cut(age, \# Create age group categories breaks = c(0, 30, 50, Inf), labels = c("young", "middle", "senior"), right = FALSE)) \|\> group_by(age_group, barrio) %\>% \# Stratify by age group and neighborhood sample_frac(0.2) \|\> \# Sample 20% of each group ungroup() \|\> dplyr::mutate(barrio_res = case_when( \# Replace barrio IDs with descriptive names barrio == "1" \~ "Iturrama", barrio == "2" \~ "San Juan/Donibane", barrio == "3" \~ "Casco Viejo/Alde Zaharra", barrio == "4" \~ "Ensanche/Zabalgunea", barrio == "5" \~ "Milagrosa-Arrosadia", barrio == "6" \~ "Rochapea/Arrotxapea", barrio == "7" \~ "Mendillorri", barrio %in% c("8", "9") \~ "Ermitagaña-Mendebaldea", barrio == "10" \~ "San Jorge-Sanduzelai", barrio == "11" \~ "Txantrea", barrio == "12" \~ "Etxabakoitz", barrio == "13" \~ "Lezkairu", barrio == "14" \~ "Buztintxuri-Euntzetxiki" ), barrio_res = as.factor(barrio_res)) \|\> dplyr::select(-age_group, -barrio)
...
```{r}
# Number of agents (replace with your actual number)
n_agents <- 167

# Extract bootstrap values for gender
gender_index <- which(names(results$t0) == "gender")
gender_bootstrap_vals <- results$t[, gender_index]

# Remove NAs (in case of failed fits)
gender_bootstrap_vals <- na.omit(gender_bootstrap_vals)

# Sample one value per agent
set.seed(123)
gender_effects <- sample(gender_bootstrap_vals, size = n_agents, replace = TRUE)

# Asign coeff only to men
agents$gender_effect <- ifelse(
  agents$gender == "1",  
  sample(gender_bootstrap_vals, size = nrow(agents), replace = TRUE),
  0  # If it is a woman, effect = 0
)
...
```{r}
# Extract bootstrap values for Nationality
nationality_index <- which(names(results$t0) == "nationality")
nationality_bootstrap_vals <- results$t[, nationality_index]

# Remove NAs (in case of failed fits)
nationality_bootstrap_vals <- na.omit(nationality_bootstrap_vals)

# Sample one value per agent
set.seed(123)
nationality_effects <- sample(nationality_bootstrap_vals, 
                              size = n_agents, 
                              replace = TRUE)


# Asign coeff only to nationals
agents$nationality_effect <- ifelse(
  agents$nationality == "1",  
  sample(nationality_bootstrap_vals, size = nrow(agents), replace = TRUE),
  0  
)
...
```{r}
# Extract bootstrap values for victimized
victimized_index <- which(names(results$t0) == "victimized.L")
victimized_bootstrap_vals <- results$t[, victimized_index]

# Remove NAs (in case of failed fits)
victimized_bootstrap_vals <- na.omit(victimized_bootstrap_vals)

# Sample one value per agent
set.seed(123)
victimized_effects <- sample(victimized_bootstrap_vals, 
                             size = n_agents, 
                             replace = TRUE)

# Asign coeff only to low victimized
agents$victimized_effect <- ifelse(
  agents$victimized == "low",  
  sample(victimized_bootstrap_vals, size = nrow(agents), replace = TRUE),
  0  
)

```

#### P7_8 "cuando ve grupos de personas haciendo botellón". Alerta

```{r}

# Extract bootstrap values for victimized
p7_8_index <- which(names(results$t0) == "p7_8.L")
p7_8_bootstrap_vals <- results$t[, p7_8_index]

# Remove NAs (in case of failed fits)
p7_8_bootstrap_vals <- na.omit(p7_8_bootstrap_vals)

# Sample one value per agent
set.seed(123)
p7_8_effects <- sample(p7_8_bootstrap_vals, 
                       size = n_agents, 
                       replace = TRUE)

# Asign coeff only to low victimized
agents$p7_8_effect <- ifelse(
  agents$p7_8 == "2",  
  sample(p7_8_bootstrap_vals, size = nrow(agents), replace = TRUE),
  0  
)
...
```{r}
agents$is_botellonero <- ifelse(agents$age >= 16 & agents$age <= 25 & runif(nrow(agents)) < 0.2, TRUE, FALSE)

```

```{r}
# Final clean-up 
agents_clean <- agents |> dplyr::select(-age) |> mutate(is_botellonero = case_when(is_botellonero == TRUE ~ "1", is_botellonero == FALSE ~ "0"))
...
Here, I merge two neighborhoods (Azpilagaña and Milagrosa-Arrosadia) from the shapefile that is later used in the GAMA model, because one of them was not included in the survey database. To ensure consistency between the spatial data and the survey data, the missing neighborhood was combined with an adjacent one that was present in the dataset. **BUSCAR MANERA DE JUSTIFICARLO, SI NO, PROXIMIDAD GEOGRÁFICA Y YA: "*Sociodemographic Similarity****: If available, reference similar sociodemographic profiles (e.g., age structure, income, housing type) between the two merged areas that are more aligned with each other than with the third alternative" –\> si miro edad o migracion no importa, juventud tendría que ser con el barrio de la izq, lo mismo para pob sin estudios, para con estudios y relacion h/m está bien así*
...
# Reading the shapefile containing neighborhood boundaries
shp <- st_read("PROY_Pol_Edificios/PROY_Pol_Edificios.shp")
...
# Saving the updated shapefile with the merged neighborhood, ready to use in GAMA
st_write(shp, "parques.shp", append = FALSE)
table(shp$building)
```
