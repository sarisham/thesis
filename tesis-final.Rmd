...
**CAMBIAR**: After carrying out the different analysis, running the CATPCA models and observed the results, it has been proven that the **quartile based encoding was the best performing recode method**:
...
pampre <- pampre |> dplyr::select(-crime_index, -p14)

table(pamplonaf$gender)
```
...
ols_step_forward_p(original_gen) # adj R2 0.506. 19 vars. gender included
ols_step_forward_aic(original_gen) # adj R2 0.499. 14 vars. gender included
ols_step_backward_aic(original_gen) # adj R2 0.499. 10 vars. gender NOT included
...
ols_step_forward_p(original_inst) #  adj R2 0.198. 11 vars. gender included
ols_step_forward_aic(original_inst) #  adj R2 0.183 11 vars. gender included
ols_step_backward_aic(original_inst) #  adj R2 0.183 14 vars. gender NOT included
...
From the results obtained, the forward-selected models showed a better performance for both the general insecurity (`gen_ins`, R-squared of 0.42, RMSE 0.77, MAE 0.63) and the institutional security (`inst_ins`, R-squared 0.07, RMSE 0.98, MAE 0.77). Therefore, this set of predictors was the one chosen for our final model.
...
# reorganizar despues: sumar barrios 8 y 9 en todo x el shapefile

We want to create a **ranking or score** of perceived (un)safety **by barrio**, using your survey distributions from:

-   `p13a`: *“Which do you think is the **safest** barrio?”*

    `p13b`: *“Which do you think is the **most unsafe** barrio?”*

Since each respondent picked **only one barrio**, we can treat the **number of times each barrio was chosen** as:

-   A **proxy for positive perception** (`p13a`)

-   A **proxy for negative perception** (`p13b`)

We can normalize these counts into proportions or scores, or even compute a **composite score** like:​
$SafetyScorei= \frac{\text{Votes}_\text{safe}(i)}{\text{Total Votes}} - \frac{\text{Votes}_\text{unsafe}(i)}{\text{Total Votes}}$

This will give you a score where:

-   High = often seen as safe, rarely seen as unsafe

-   Low = often seen as unsafe, rarely seen as safe

```{r}
# Raw vote counts
votes_safe <- c(199, 88, 52, 249, 1, 20, 23, 74, 5, 48, 1, 53, 15)
votes_unsafe <- c(3, 29, 137, 5, 211, 127, 3, 3, 196, 27, 67, 0, 20)
...
# Optional: rank them
barrio_scores$safety_rank <- rank(-barrio_scores$safety_score)
summary(barrio_scores$safety_score)
...
# tengo que sumar esas dos categorías de barrio que iban juntas. 
...
agents <- pampre %>%
  ...
  mutate(age_group = cut(age,
                         ...
                         right = FALSE)) |> 
  group_by(age_group, barrio) %>%
  sample_frac(0.2) |> 
  ungroup() |> 
  dplyr::mutate(barrio_res = case_when(barrio == "1" ~ "Iturrama",
                                   ...
                                   barrio %in% c("8", "9") ~ "Ermitaga�a-Mendebaldea",
...
agents_clean <- agents
rownames(agents_clean) <- NULL  # elimina nombres de fila
agents_clean <- as.data.frame(agents_clean, stringsAsFactors = FALSE)
colnames(agents_clean)  # revisa que estén bien
...
# Saving the updated shapefile with the merged neighborhood
