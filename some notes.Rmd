...
"Given the different data types (binary, ordinal, nominal, numeric), it
is generally advisable to impute each type separately to ensure that the
methods are appropriate for the data type. However, if you face issues
with mixed data types or find that a unified approach (like using cart)
works without errors, that can be a pragmatic solution. Here's the
decision-making process:

Consistency Across Types: Using different methods (e.g., logreg for
binary, polr for ordinal, pmm for numeric) is ideal for accuracy.
However, if this leads to complications or errors, and the cart method
successfully imputes all types, then using cart could be a practical
alternative. Unified Approach with cart: The cart method (Classification
and Regression Trees) is versatile and can handle different types of
data, which makes it a safe choice when mixed data types are present."

"Imputation Steps with cart Since using cart works without issues:

Advantages: It avoids the complexity of matching methods to data types
and potential errors from inappropriate method applications.
Considerations: While cart can impute different types of variables, be
mindful of the interpretation, as it may treat all data as numeric-like
under the hood, which could influence how categorical relationships are
modeled."

"Handling NAs and Outliers: Your approach to NA values and outliers
(such as converting specific values to NA) is crucial. Make sure to
document these transformations thoroughly."

"Bivariate Analysis: Your use of chi-square tests, t-tests, and ANOVA is
appropriate for exploring relationships between variables. Ensure that
assumptions for these tests (like normality for t-tests) are met or note
any deviations."
...
Cross-validation is applied to validate this imputation model (why).
This involves partitioning the data into training and test sets multiple
times to assess the stability and accuracy of the imputation. Then, we
will check post-imputation data distributions:

ah alright so from the crossvalidaation i dont need to get a new dataset
with the imputed data, but rather compare the variability across the
resulting cross by checking different parameters and see how consistent
they are, right? after that, once we have determined
consistency/robustness we can actually use (or not) cart to impute de
data. right? es, exactly. The primary purpose of cross-validation in the
context of imputation is to assess the consistency and robustness of the
imputation method. Here's a step-by-step outline:

Cross-Validation Process:

-   Split the Data: The dataset is divided into multiple folds. For each
    fold, a subset of the data is used as the training set (to perform
    imputation), and the remaining subset is used as the test set.
-   Impute Missing Data: For each fold, the missing data in the training
    set is imputed using the selected method (e.g., cart).
-   Assess Imputation Consistency: After imputing, we compare the
    imputed values across different folds. Consistency across folds
    suggests that the method is robust and reliable. Evaluating
    Consistency:

"Parameter Checking: Look at various parameters or metrics that are
critical to your analysis. For example, you can check the mean, standard
deviation, or distribution of imputed values across different folds.
Bias and Variance: Assess whether the imputed values are systematically
biased (e.g., consistently higher or lower than expected) or have high
...
1.  Extract Imputed Values Across Folds First, extract the imputed
    datasets from each fold and compare them:
...
2.1. For numeric variables: You can calculate statistics like the mean,
standard deviation, and other relevant metrics for each variable to see
how consistent the imputed values are across different folds.
...
2.2. Analyzing variability for categorical variableS: For categorical
variables, we typically use frequency tables and visualizations like bar
plots to analyze the consistency and variability of the imputed values.

Calculate Frequency Distributions To assess the variability, you can
calculate the frequency distribution of each category in the imputed
data and compare it with the original data
...
3.  Visualize Distribution of Imputed Values to see if there are any
    significant differences across folds. 3.1. Numeric variables
...
3.2. Visualizing Categorical Data Distribution Plotting Bar Charts for
Categorical Data For categorical data, use bar charts to compare the
original and imputed distributions.

could this be due to the fact that the combined imputed data, after the
cross validation process, has 4095 observations, whereas the original
dataset has only 828? because although the frequencies are diff., the
distributions seems quite similar

You're correct in observing that the frequencies in the bar plot appear
different, which can indeed be attributed to the difference in the
number of observations between the original and imputed datasets. In
cross-validation, when you combine imputed data from all folds, you
effectively increase the number of observations because you're
aggregating data across multiple folds.

Understanding the Impact of Cross-Validation on Data Size Original vs.
Combined Data Size:

The original dataset has 828 observations. The combined imputed dataset
from cross-validation has 4095 observations, likely due to combining
imputed datasets from each fold, resulting in a higher total count.
...
The normalization process corrects for differences in sample size,
allowing you to focus on the shape and distribution of the data rather
than absolute frequencies. This will help you confirm whether the
imputation has maintained the relative proportions across categories or
introduced any significant distortions.

Since the imputed values are consistent and align well with the
non-missing data, the method is considered robust and it is
validated.Therefore, based on the consistency and robustness assessment,
we decide to proceed with cart for the final imputation of the entire
dataset.

*Carrying out the imputation*: 1. Imputation without Cross-Validation
Imputing the entire dataset using cart without cross-validation is
straightforward and often sufficient if:

-   Validation is Complete: You've already validated the cart method
    through previous cross-validation steps and found it robust and
    consistent.
-   Simplicity and Efficiency: This approach is simpler and faster, as
    it avoids the complexity of combining results from multiple folds.
...
-   Now we focus in the variables we identified might be related to the
    *criminality index*. MIRAR SI INCLUIR LOS DATOS DE LA POLICÍA En el
    cuestionario, de esto se habla en: P5. Incidentes de robo y atraco,
    P6. Experiencias de violencia y acoso, P12. Percepción de
    degradación y delincuencia en el barrio, P16. Conflictos de
    convivencia en el barrio donde vive, P17. Motivos del conflicto en
    el barrio.
...
-   Now the *perception index* En el cuestionario, de esto se habla en:
    P7: Percepción de seguridad en diferentes espacios, P9: Cambio de
    domicilio por inseguridad en el barrio, algunas de P10: Opiniones
    sobre la seguridad al caminar solo/a, evitar zonas, cámaras de
    seguridad, P11: Nivel de seguridad en barrio y ciudad, P12.
    Percepción de degradación y delincuencia en el barrio (tendría que
    decidir en qué índice incluyo esta variable). Quizás también P14:
    Valoración de los servicios de la Policía Municipal.
...
-   Scaling and Normalization: Adjusting the scale of features to ensure
    they have a similar range, which can improve model performance,
    especially for algorithms that are sensitive to feature scaling.
-   Encoding Categorical Variables: Converting categorical features into
    numerical values that models can interpret, such as one-hot encoding
    or label encoding.

Feature Selection:

-   Choosing Relevant Features: Identifying and selecting the most
    relevant features that contribute to the model's predictive power,
    often using techniques like correlation analysis, feature importance
    from models, or statistical tests.
-   Dimensionality Reduction: Techniques like Principal Component
    Analysis (PCA) or Feature Selection can reduce the number of
    features, simplifying the model and potentially improving
    performance.

# REGRESSIONS

Lo de stats I q tengo q ver cómo llamarlo claro no puedo hacer lineales.
tendrán q ser logaritmicas o alfgo asi
...
Creo que esto no habria que hacerlo porque de hecho empeora el modelo
xddd. Así que lo voy a dejar como está, pero dejo el código por si
acaso. (Recodifica 1 como 1 y 2-3 como 2).

```{r eval=FALSE, include=FALSE}
# Definir la función de recodificación
rec <- function(variable) {
  ifelse(variable == 1, 1, 
         ifelse(variable %in% 2:3, 2, variable))
}

# Aplicar la función a las variables de percepción
perception_vars$p7_1 <- rec(perception_vars$p7_1)
perception_vars$p7_2 <- rec(perception_vars$p7_2)
perception_vars$p7_3 <- rec(perception_vars$p7_3)
perception_vars$p7_4 <- rec(perception_vars$p7_4)
perception_vars$p7_5 <- rec(perception_vars$p7_5)
perception_vars$p7_6 <- rec(perception_vars$p7_6)
perception_vars$p7_8 <- rec(perception_vars$p7_8)
